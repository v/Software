...
class LineDetectorNode(object):
    def __init__(self):
        ...
        self.pub_image_filtered_white = rospy.Publisher("~image_filtered_white", Image, queue_size=1)
        self.pub_image_filtered_yellow = rospy.Publisher("~image_filtered_yellow", Image, queue_size=1)
        self.pub_image_filtered_red = rospy.Publisher("~image_filtered_red", Image, queue_size=1)
...
#             self.detector_config = c
    ...
    def processImage_(self, image_msg):
        ...
        white_filtered = self.detector.detectLines2('white')
        yellow_filtered = self.detector.detectLines2('yellow')
        red_filtered = self.detector.detectLines2('red')


        white_image_msg_out = self.bridge.cv2_to_imgmsg(white_filtered, "8UC1")
        white_image_msg_out.header.stamp = image_msg.header.stamp
        self.pub_image_filtered_white.publish(white_image_msg_out)

        yellow_image_msg_out = self.bridge.cv2_to_imgmsg(yellow_filtered, "8UC1")
        yellow_image_msg_out.header.stamp = image_msg.header.stamp
        self.pub_image_filtered_yellow.publish(yellow_image_msg_out)

        red_image_msg_out = self.bridge.cv2_to_imgmsg(red_filtered, "8UC1")
        red_image_msg_out.header.stamp = image_msg.header.stamp
        self.pub_image_filtered_red.publish(red_image_msg_out)
